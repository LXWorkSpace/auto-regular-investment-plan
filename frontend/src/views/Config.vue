  ...
  <div class="config-container">
    <el-card shadow="hover" class="config-card">
      ...
      >
        ...
        <el-form-item label="风险偏好" prop="risk_preference">
          <el-radio-group v-model="configStore.config.risk_preference">
            <el-radio label="低风险">低风险</el-radio>
            <el-radio label="中等风险">中等风险</el-radio>
            <el-radio label="高风险">高风险</el-radio>
          </el-radio-group>
        </el-form-item>
        
        <el-form-item label="资金池缓冲比例" prop="buffer_percentage">
          <el-tooltip content="预留资金百分比，用于紧急情况和下跌时加仓">
            <el-slider 
              v-model="configStore.config.buffer_percentage"
              :min="0"
              :max="0.3"
              :step="0.01"
        ...
        <div class="assets-config">
          <div class="assets-header">
            <h3>已添加资产 <el-tag type="info">总权重: {{ percentageFormatter(configStore.totalWeight) }}</el-tag></h3>
            <div>
              <el-button type="primary" size="small" @click="showAssetDialog" plain>
                添加资产
              </el-button>
              <el-button 
                type="success" 
          ...
          <el-alert
            v-if="configStore.config.assets.length > 0 && Math.abs(configStore.totalWeight - 1) > 0.001"
            type="warning"
            :closable="false"
            show-icon
            title="资产权重总和应等于100%"
        ...
        <el-tab-pane label="自定义资产" name="custom">
          <el-form :model="newAsset" label-position="top">
            ...
            <el-form-item label="描述">
              <el-input v-model="newAsset.description" type="textarea" rows="2" />
            </el-form-item>
          </el-form>
        </el-tab-pane>
      </el-tabs>
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button 
            type="primary" 
            @click="addAsset"
            :disabled="assetDialogTab === 'custom' && (!newAsset.name || !newAsset.code || !newAsset.type)"
          >
            添加
          </el-button>
        </div>
...
const rules = {
  monthly_investment: [
    { required: true, message: '请输入月度投资金额', trigger: 'blur' },
    { type: 'number', min: 100, message: '金额必须大于等于100', trigger: 'blur' }
  ],
  risk_preference: [
...
const saveConfig = async () => {
  if (!configStore.isValidConfig) {
    ElMessage.warning('请确保总权重为100%且月度投资金额已设置')
    return
  }
